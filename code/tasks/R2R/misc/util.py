...
def build_angle_features(heading, elevation, size):
...
def build_action_embeddings(adj_loc_list, features, size):
    feature_dim = features.shape[-1]
    embedding = np.zeros((len(adj_loc_list), feature_dim + size),
        dtype=np.float32)
    for a, adj_dict in enumerate(adj_loc_list):
        ...
        embedding[a, feature_dim:(feature_dim + size)] = build_angle_features(
            rel_heading, rel_elevation, size)

    return embedding

def calculate_headings_and_elevations_for_views(sim, goalViewIndices):
    states = sim.getState()
    heading_deltas = []
    elevation_deltas = []
