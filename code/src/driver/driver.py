...
sim.setNavGraphPath(os.path.join('../data/navask', 'connectivity'))
...
while True:
    ...
    if state.location.viewpointId != vp_id or abs(state.heading - heading_id) > 1e-6 \
       or abs(state.elevation - elevation_id) > 1e-6:
        ...
        print(vp_id, heading_id, elevation_id, state.viewIndex, state.location.x, state.location.y, state.location.z)
