...
from models.vlnbert.vlnbert_init import get_vlnbert_models
...
class VLNBERT(nn.Module):
    def __init__(self, config):
        ...
        self.args = config.nav_agent.model

        feature_size = self.args.image_feat_size + self.args.angle_feat_size

        self.vln_bert = get_vlnbert_models(self.args, config=None)  # initialize the VLN-BERT
        ...
        self.action_state_project = nn.Sequential(
            nn.Linear(hidden_size+self.args.angle_feat_size, hidden_size), nn.Tanh())
        self.action_LayerNorm = BertLayerNorm(hidden_size, eps=layer_norm_eps)

        self.drop_env = nn.Dropout(p=self.args.featdropout)
    ...
    def forward(self, mode, sentence, token_type_ids=None,
        ...
        if mode == 'language':
            ...
            cand_feats[..., :-self.args.angle_feat_size] = self.drop_env(cand_feats[..., :-self.args.angle_feat_size])
